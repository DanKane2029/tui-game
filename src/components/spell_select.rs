...
use crate::model::{GameEvent, InputEvent, Spell, globals::PLAYER_SPELL_SLOTS};
...
impl SpellSelect {
    pub fn new(
        spells: [Option<Spell>; 5],
        input_receiver: Receiver<InputEvent>,
        game_event_sender: Sender<GameEvent>,
    ) -> Self {
        let layout_constrains = (0..spells.len())
            .map(|_| Constraint::Percentage(100 / spells.len() as u16))
        ...
        if let Ok(input_event) = self.input_receiver.try_recv() {
            match input_event {
                ...
                InputEvent::Enter => {
                    ...
                    if let Some(spell) = selected_spell {
                ...
                Some(spell) => {
                    f.render_widget(
                        Paragraph::new(format!(
                            "damage: {}\nblock: {}",
                            spell.damage_value, spell.block_value
                        ))
                        .block(
                            Block::default()
                                .borders(Borders::all())
                                .title(spell.name.clone())
                                .style(if index == self.cur_select_index {
