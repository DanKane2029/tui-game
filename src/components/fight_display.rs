        ...
        let layout_map = HashMap::from([
            ...
            (
                ...
                Layout::default()
                    .direction(Direction::Horizontal)
                    .constraints(
                        (0..num_enemies).map(|_| Constraint::Percentage(100 / num_enemies as u16)),
                    ),
            ),
            (
                "enemy",
...
impl Component for FightDisplay {
    ...
    fn render(&mut self, f: &mut ratatui::Frame, rect: ratatui::prelude::Rect) {
        ...
        let player_rects = self
            .layout_map
            .get("player")
            .unwrap()
            .split(fight_display_rects[0]);

        f.render_widget(
            Block::default().title("Player Pic").borders(Borders::all()),
            player_rects[0],
        );

        self.player_stats_display.render(f, player_rects[1]);

        let enemies_rects = self
            .layout_map
            .get("enemies")
            .unwrap()
            .split(fight_display_rects[1]);
        ...
        for (index, enemy_rect) in enemies_rects.to_vec().iter().enumerate() {
            ...
            f.render_widget(
                Block::default().title(enemy.name).borders(Borders::all()),
                enemy_rect[0],
            );
